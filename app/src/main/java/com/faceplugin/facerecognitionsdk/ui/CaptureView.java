...
public class CaptureView extends View implements Animator.AnimatorListener, ValueAnimator.AnimatorUpdateListener {
    ...
    public void setCapturedBitmap(Bitmap bitmap) {
        ...
        getROIRect1(frameSize).round(roiRectSrc);
        ...
        canvas1.translate(roiRectSrc.width(), 0);
        canvas1.scale(-1f, 1f);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        RectF roiViewRect = mapRoiToView(canvas.getWidth(), canvas.getHeight());
        ...
        if((viewMode == VIEW_MODE.TO_FACE_CIRCLE && animateValue < 1.0f) || viewMode == VIEW_MODE.FACE_CIRCLE_TO_NO_FACE) {
            ...
            if(faceBoxes != null && faceBoxes.size() > 0) {
                ...
                double yaw = mirrorX ? faceBox.yaw : -faceBox.yaw;
                // Tick marks use mathematical Y-up (centerY - y). Pitch must too:
                // Android Y-down + engine pitch would lean the oval the wrong way.
    ...
    private RectF mapRoiToView(float viewW, float viewH) {
        RectF roiRect = getROIRect1(frameSize);
        float ratioView = viewW / viewH;
        float ratioFrame = frameSize.getWidth() / (float) frameSize.getHeight();
        float dx;
        float dy;
        float ratio = viewH / (float) frameSize.getHeight();
        if (ratioView < ratioFrame) {
            dx = ((viewH * ratioFrame) - viewW) / 2f;
            dy = 0f;
        } else {
            dx = 0f;
            dy = ((viewW / ratioFrame) - viewH) / 2f;
        }
        return new RectF(
                roiRect.left * ratio - dx,
                roiRect.top * ratio - dy,
                roiRect.right * ratio - dx,
                roiRect.bottom * ratio - dy);
    }
