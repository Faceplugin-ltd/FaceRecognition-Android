...
object FaceJson {
    ...
    fun toFaceBox(face: VideoWorkerFace): FaceBox {
        ...
        box.liveness = face.liveness
        box.livenessLabel = face.livenessLabel.orEmpty()
    ...
    private fun parseVideoWorkerFaces(faces: JSONArray?): List<VideoWorkerFace> {
        ...
        return buildList {
            for (i in 0 until faces.length()) {
                ...
                val pose = face.optJSONObject("facePose")
                val (liveness, livenessLabel) = parseWorkerLiveness(face)
                add(
                    VideoWorkerFace(
    ...
    private fun parseWorkerLiveness(face: JSONObject): Pair<Float, String?> {
        var label = face.optString("livenessLabel").ifBlank {
            face.optString("liveness_label")
        }
        var score = doubleValue(face.opt("liveness"))
        val attrs = face.optJSONObject("attributes")
        val live = attrs?.optJSONObject("Liveness2D") ?: attrs?.optJSONObject("liveness")
        if (live != null) {
            if (label.isBlank()) label = live.optString("value")
            if (score == null) {
                score = doubleValue(live.opt("confidence")) ?: doubleValue(live.opt("value"))
            }
        }
        if (label.isBlank()) label = null
        return (score?.toFloat() ?: 0f) to label
    }
